...
class IMPress_Agents {
	...
	function __construct() {
		...
		$this->employee_details = apply_filters(
			'impress_agents_employee_details',
			array(
				'col1' => array(
		...
		$this->employee_social = apply_filters(
			'impress_agents_employee_social',
			array(
	...
	function create_post_type() {
		...
		$args = apply_filters(
			...
			array(
				...
				'rewrite'               => array(
					'slug'       => $this->options['impress_agents_slug'],
					'feeds'      => true,
					'with_front' => false,
				),
			)
	...
	function columns_data( $column ) {
		...
		switch ( $column ) {
			...
			case 'employee_tags':
				...
				_e( '<b>Office:</b> ' . get_the_term_list( $post->ID, 'offices', '', ', ', '' ) . '<br />', 'impress_agents' );
				break;
		}
